...
public abstract class LanguageLexerBase : Lexer
{
    public LanguageLexerBase(ICharStream input)
        : base(input)
    {
        _input = input;
    }

    public LanguageLexerBase(ICharStream input, TextWriter output, TextWriter errorOutput)
    ...
    protected void OnColon()
    {
        ...
        if (interpolatedStringLevel > 0)
        {
            ...
            {
                this.Mode(LanguageLexer.INTERPOLATION_FORMAT);
            }
        }
    }
